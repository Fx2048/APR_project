...
class Chess(gym.Env):
    ...
    def draw_piece(self, x: int, y: int) -> None:
        ...
        for color in [Pieces.BLACK, Pieces.WHITE]:
            ...
            yy = abs((color * 7) - y)
            piece_ascii = Pieces.get_ascii(color, self.board[color, row, col])

            pos_x, pos_y = self.get_left_top(x, yy, offset=self.cell_size // 2)

            # Solo aplicar efecto si es pieza blanca
            if color == Pieces.WHITE:
                # Borde blanco (efecto contorno)
                for dx in [-1, 1]:
                    for dy in [-1, 1]:
                        border_text = self.font.render(piece_ascii, True, Colors.WHITE)
                        rect = border_text.get_rect()
                        rect.center = (pos_x + dx, pos_y + dy)
                        self.screen.blit(border_text, rect)

                # Texto negro (relleno)
                main_text = self.font.render(piece_ascii, True, Colors.BLACK)
                rect = main_text.get_rect()
                rect.center = (pos_x, pos_y)
                self.screen.blit(main_text, rect)
            else:
                # Para piezas negras normales (blancas sin borde)
                text = self.font.render(piece_ascii, True, Colors.WHITE)
                rect = text.get_rect()
                rect.center = (pos_x, pos_y)
    ...
    def encode_action(self, from_pos: tuple[int, int], to_pos: tuple[int, int]) -> int:
        """
        Codifica un movimiento (from, to) en un índice único [0-639]
        """
        return (from_pos[0] * 8 + from_pos[1]) * 8 * 8 + (to_pos[0] * 8 + to_pos[1])

    def decode_action(self, action: int) -> tuple[tuple[int, int], tuple[int, int]]:
        """
        Decodifica el índice en una tupla (from, to)
        """
        from_idx = action // 64
        to_idx = action % 64
